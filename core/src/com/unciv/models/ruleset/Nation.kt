...
class Nation : INamed, CivilopediaText() {
    ...
    override fun getCivilopediaTextHeader() = FormattedLine(name, icon="Nation/$name", header=2)
    ...
    override fun getCivilopediaTextLines(ruleset: Ruleset): List<FormattedLine> {
        ...
        if (leaderName.isNotEmpty()) {
            textList += FormattedLine(getLeaderDisplayName(), centered=true, header=4)
            textList += FormattedLine( extraImage="LeaderIcons/$leaderName", imageSize=300f )
            textList += FormattedLine()
        }

        if (uniqueName != "") textList += FormattedLine("{$uniqueName}:", bold=true)
        if (uniqueText != "") {
        ...
        if (startBias.isNotEmpty()) {
            ...
            startBias.forEach {
                ...
                textList += FormattedLine((if (first) "{Start bias:} " else "") + it.tr(), link="Terrain/$link", indent=if(first) 0 else 1)
            }
            textList += FormattedLine()
        }
        addUniqueBuildingsText(textList, ruleset)
    ...
    @JvmName("addUniqueBuildingsText1")
    ...
    private fun addUniqueUnitsText(textList: ArrayList<FormattedLine>, ruleset: Ruleset) {
        for (unit in ruleset.units.values) {
            ...
            if (unit.replaces != null && ruleset.units.containsKey(unit.replaces!!)) {
                ...
                for (unique in unit.uniques.filterNot { it in originalUnit.uniques })
                    textList += FormattedLine(Translations.translateBonusOrPenalty(unique), indent=1)
                for (unique in originalUnit.uniques.filterNot { it in unit.uniques })
                    textList += FormattedLine("Lost ability".tr() + "(" + "vs [${originalUnit.name}]".tr() + "): " +
                            Translations.translateBonusOrPenalty(unique), indent=1)
                for (promotion in unit.promotions.filter { it !in originalUnit.promotions })
                    textList += FormattedLine( promotion.tr() + " (" +
    ...
    private fun addUniqueImprovementsText(textList: ArrayList<FormattedLine>, ruleset: Ruleset) {
        for (improvement in ruleset.tileImprovements.values) {
            ...
            textList += FormattedLine(improvement.name)
